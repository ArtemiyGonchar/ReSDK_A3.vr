...
class(GMTestingR) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMTestingR
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMTestingR.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMTestingR
	getterconst_func(getMapName,"fak");
	...
	// Список ролей (имён классов) для режима GMTestingR, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
            ...
            /*
                Пример:
                    "GMTestingR_BasicRole1",
                    "GMTestingR_BasicRole2",
                    "GMTestingR_BasicRole3"
            */
		]
	};
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMTestingR_BasicRole1" встало 2 игрока и на "GMTestingR_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMTestingR_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMTestingR_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMTestingR"
	};
	...
	// Вызывается когда режим GMTestingR выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMTestingR
	func(onFinish)
	{
		objParams();
