...
// Базовая роль для режима GMTestingR. Рекомендуется именовать все дочерние роли для этого режима с постфиксом имени режима и префиксом R (Role)
// Пример: RCaveExplorer_GMTestingR
class(GMTestingR_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
	...
	// Установите true требуется для тех ролей, без которых режим GMTestingR не сможет полноценно функционировать.
...
/*
	Определяем роли, унаследованные от GMTestingR_BasicRole

class(RDebugRole_GMTestingR) extends(GMTestingR_BasicRole)
	//change if need
	var(name,"Debug role");
