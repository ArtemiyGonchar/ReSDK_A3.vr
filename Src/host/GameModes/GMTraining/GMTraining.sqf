...
class(GMTraining) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMTraining
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMTraining.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMTraining
	getterconst_func(getMapName,"training");
	...
	// Список ролей (имён классов) для режима GMTraining, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
            ...
            /*
                Пример:
                    "GMTraining_BasicRole1",
                    "GMTraining_BasicRole2",
                    "GMTraining_BasicRole3"
            */
		]
	};
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMTraining_BasicRole1" встало 2 игрока и на "GMTraining_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMTraining_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMTraining_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMTraining"
	};
	...
	// Вызывается когда режим GMTraining выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMTraining
	func(onFinish)
	{
		objParams();
