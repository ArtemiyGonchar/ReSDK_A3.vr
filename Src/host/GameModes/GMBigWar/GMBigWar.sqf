...
class(GMBigWar) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMBigWar
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMBigWar.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMBigWar
	getterconst_func(getMapName,"BigWar");
	...
	// Список ролей (имён классов) для режима GMBigWar, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMBigWar_BasicRole1" встало 2 игрока и на "GMBigWar_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMBigWar_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMBigWar_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMBigWar"
	};
	...
	// Вызывается когда режим GMBigWar выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMBigWar
	func(onFinish)
	{
		objParams();
